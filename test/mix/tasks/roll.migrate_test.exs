defmodule Mix.Tasks.Roll.MigrateTest do
  use ExUnit.Case

  import Mix.Tasks.Roll.Migrate, only: [run: 2]
  import Support.FileHelpers

  @migrations_path Path.join([tmp_path(), inspect(Roll.Migrate), "roll"])
  ...
  test "runs the migrator with app_repo config" do
    Application.put_env(:roll, :ecto_repos, [Repo])
  ...
  test "runs the migrator with --step" do
    run(["-r", to_string(Repo), "-n", "1"], fn repo, [path], direction, opts ->
      ...
      refute path =~ ~r/_build/
      assert direction == :up
      assert opts == [repo: "Elixir.Mix.Tasks.Roll.MigrateTest.Repo", step: 1]
      []
    end)
  ...
  test "uses custom paths" do
    path1 = Path.join([unquote(tmp_path()), inspect(Roll.Migrate), "migrations_1"])
    path2 = Path.join([unquote(tmp_path()), inspect(Roll.Migrate), "migrations_2"])
    File.mkdir_p!(path1)
    File.mkdir_p!(path2)
